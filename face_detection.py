...
Original file is located at
    https://colab.research.google.com/drive/1Jk4-qX9zdYGsBrTnh2vF52CV9ucuqpjk
...
Our dataset is provided by [
Dataturks](https://dataturks.com/), and it is hosted on [Kaggle](https://www.kaggle.com/dataturks/face-detection-in-images). Here's an excerpt from the description:
...
for index, row in tqdm(faces_df.iterrows(), total=faces_df.shape[0]):
    original_image_name = row['file_upload']
    img_path = "http://127.0.0.1:8080"+row["data"]["image"]
    img = urllib.request.urlopen(img_path)
    img = Image.open(img)
    img = img.convert('RGB')

    image_name = f'{original_image_name}.jpeg'

    img.save(f'faces/{image_name}', "JPEG")

    img_show = cv2.cvtColor(cv2.imread(f'faces/{image_name}'), cv2.COLOR_BGR2RGB)
    # img_show = cv2.resize(img_show, (100, 100), interpolation=cv2.INTER_AREA)
    annotations = row["annotations"]
    width = 0
    height = 0
    for annotation in annotations:
      for an in annotation["result"]:
        data = {}

        width = an['original_width']
        height = an['original_height']
        value = an['value']
        ...
        x_min = int(round(value["x"] * x_ratio))
        y_min = int(round(value["y"] *y_ratio))
        box_width = int(round(value['width'] * x_ratio))
        box_height = int(round(value['height'] * y_ratio))
        ...
        img_show = cv2.rectangle(img_show, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)

        data['class_name'] = value['rectanglelabels']
        dataset.append(data)

    img_show = cv2.resize(img_show, (width, height), interpolation=cv2.INTER_AREA)
    plt.imshow(img_show)
    plt.axis('off')
    plt.show()
    break
